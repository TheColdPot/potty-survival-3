...
execute if entity @s[tag=anvil_has_item] if data entity @a[limit=1,tag=anvil_player] SelectedItem.components."minecraft:custom_data".is_attachable unless score @s animation_frame matches 0.. run function potty.items:anvil/attach/copy_and_start
