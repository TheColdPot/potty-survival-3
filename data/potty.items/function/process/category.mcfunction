...
execute store result score #gear_type var run data get storage potty.items:constructor target.components.minecraft:custom_data.gear_type

execute if score #gear_type var matches 100 run data modify storage potty.items:constructor gear_type_name set value '["",{"translate":"potty.gear.sword.space","font":"potty:spaces","color":"#424242","fallback":""},{"translate":"potty.gear.sword","fallback":" sᴡᴏʀᴅ ","color":"gray"}]'
...
execute store result score #gear_rarity var run data get storage potty.items:constructor target.components.minecraft:custom_data.rarity
